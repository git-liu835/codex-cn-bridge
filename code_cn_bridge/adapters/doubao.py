"""豆包 (Doubao) 适配器 —— 火山引擎 Ark API, 支持 Seed 2.0 系列

Seed 2.0 Pro (2026-02):
  - 上下文: 256K, max_output: 128K
  - thinking: 专家模式 (Expert Mode), API 参数文档不透明
  - 通过 thinking.type 控制: enabled / disabled
  - 不支持 budget_tokens

Chat:   POST /api/v3/chat/completions  (OpenAI 兼容)
Images: POST /api/v3/images/generations (DALL-E 兼容, Seedream)
"""
...
class DoubaoAdapter(BaseAdapter):
    ...
    supports_thinking_budget: bool = False  # 豆包不支持 budget_tokens
    ...
    def apply_thinking(self, chat_req: dict) -> dict:
        ...
        豆包 API 文档不透明, 使用 thinking.type 标准控制。
        """
